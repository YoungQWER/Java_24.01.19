...
//	}
	...
	public Movie(String title, String gener) {
		this(Instant.now().getEpochSecond(), title, gener);
		
		Long id = Instant.now().getEpochSecond();     //영화 고유번호
	...
	public static ArrayList<Movie> findAll(){
		...
		try {
			br = new BufferedReader(new FileReader(file));   //file 없으면 어쩔건데 (예외처리)
...
//	}
	...
	public void save() {
		try {
								//movies.txt 파일에 이어서 쓰기 설정. false면 덮어쓴다.
